...
void subtract_counter(std::vector<node*>& vec,node*const& nd)
{
    ...
    for(auto it = vec.begin(); it != vec.end();) //go over parents node besides root, if they have counter then subtract by 1
        if((*it)->key <= nd->key) //it's right child just erase from vector 
            it = vec.erase(it++); 
        else
        {
...
int main()
{
    ...
    recursive_insert(root,19);
    recursive_insert(root,16);
    recursive_insert(root,13);
    recursive_insert(root,14);
    recursive_insert(root,18);
    //recursive_insert(root,18);
    ...
    recursive_remove(root,18);
    recursive_remove(root,16);
    recursive_remove(root,4);
    recursive_remove(root,2);
    recursive_remove(root,14);
    recursive_remove(root,1);
    //recursive_remove(root,1);
    //recursive_remove(root,16);
    //recursive_remove(root,14);
    //recursive_remove(root,5);
